 ...
 *
 * Demonstrates how to use pipe with map() to buffer URLs until complete.
...
/**
 * Closure state for URL buffering.
 ...
 */
const processBuffer = (patterns: Array<BufferPattern>): Array<UIMessageChunk> => {
  ...
  return pipe(stream)
    .map(({ chunk }) => {
      ...
      if (chunk.id !== currentId) {
        ...
        currentId = chunk.id;
        buffer = chunk.delta;
        const result = processBuffer(patterns);
